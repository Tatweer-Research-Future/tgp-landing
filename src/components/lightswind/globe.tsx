...
interface GlobeProps {
  ...
  markers?: Marker[]; // New prop for custom marker control
}
...
const Globe: React.FC<GlobeProps> = ({
  ...
  useEffect(() => {
    ...
    // Build cobe markers array if markers prop is set
    const cobeMarkers =
      Array.isArray(markers) && markers.length > 0
        ? markers.map((m) => {
            const rgb = m.color
              ? hexToRgbNormalized(m.color)
              : resolvedMarkerColor;
            // Optionally support per-marker size (default 0.15)
            return {
    ...
    markerColor,
    glowColor,
    markers, // Listen for markers prop changes
  ]);
  ...
  return (
    ...
    >
      <canvas
        ref={canvasRef}
        style={{
          width: "45rem", // Canvas takes full width of its parent (which is constrained)
          height: "45rem", // Canvas takes full height of its parent (which is constrained)
